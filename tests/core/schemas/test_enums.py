...
class TestFileFormatFromMime:
    ...
    def test_audio_mime_returns_correct_format(self):
        assert FileFormat.from_mime("audio/mpeg") == FileFormat.MP3
        assert FileFormat.from_mime("audio/wav") == FileFormat.WAV

    def test_image_mime_returns_correct_format(self):
        assert FileFormat.from_mime("image/png") == FileFormat.PNG
        assert FileFormat.from_mime("image/jpeg") == FileFormat.JPEG

    def test_video_mime_returns_correct_format(self):
        assert FileFormat.from_mime("video/mp4") == FileFormat.MP4
        assert FileFormat.from_mime("video/webm") == FileFormat.WEBM
