...
class TestTokenBudgetStrategy:
    ...
    def test_no_user_messages_returns_history_unchanged(self):
    ...
    def test_trim_drops_oldest_turns_over_budget(self):
    ...
    def test_always_keeps_most_recent_turn_even_if_over_budget(self):
...
class TestSummarizingStrategy:
    ...
    def test_summarizes_dropped_turns_into_system_message(self):
        llm = MagicMock()
    ...
    def test_summarizes_dropped_tool_messages_too(self):
        llm = MagicMock()
        llm.generate.return_value = LLMResponse(
        ...
        transcript = llm.generate.call_args.kwargs["prompt"].last_user_message().text
        assert "tool output" in transcript

    def test_summary_unavailable_when_llm_returns_no_message(self):
        llm = MagicMock()
        llm.generate.return_value = LLMResponse(
            message=None,
            usage=TokenUsage(),
