...
async def test_extract_returns_chunks_for_given_document_id():
    ...
    with patch.object(TesseractOCR, "_load_image", return_value="fake-image"), \
         patch("integrations.ocr.tesseract.pytesseract.image_to_data", return_value=FAKE_TESSERACT_DATA):
...
async def test_extract_applies_min_confidence_from_config():
    ...
    ocr = TesseractOCR(TesseractConfig(min_confidence=0.9))
    ...
    # only "Total:" (0.91) clears the 0.9 threshold, "42.00" (0.87) is dropped
