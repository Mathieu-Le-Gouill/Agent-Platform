...
class KMeansConfig(ClusteringConfig):
    ...
    n_init: int | Literal["auto"] = 10  # number of times the algorithm runs with different centroid seeds; best result (by inertia) is kept
    ...
    algorithm: Literal["lloyd", "elkan"] = "lloyd"  # K-means variant to use; "elkan" is faster on well-defined clusters via triangle inequality
    random_state: int | None = None  # seed controlling centroid initialization for reproducible results
