...
class HDBSCANConfig(ClusteringConfig):
    ...
    min_samples: int | None = None  # neighborhood size used to estimate density; higher values yield more conservative clustering, defaults to min_cluster_size
    ...
    metric: str = "euclidean"  # distance metric used to compute pairwise distances between points
    ...
    alpha: float = 1.0
    allow_single_cluster: bool = False  # if True, permits the entire dataset to be returned as a single cluster
    cluster_selection_epsilon_max: float = float("inf")  # distance threshold above which cluster splits are no longer merged back together
