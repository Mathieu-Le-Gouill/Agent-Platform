...
class LangChainVectorStore(BaseVectorStore):
    ...
    async def search(self, query_vector: list[float], k: int = 5) -> list[TextChunk]:
        results = await asyncio.to_thread(
            self._client.similarity_search_by_vector, query_vector, k,
        )
    ...
    async def search_with_scores(self, query_vector: list[float], k: int = 5) -> list[tuple[TextChunk, Score]]:
        results = await asyncio.to_thread(
            self._client.similarity_search_with_score, query_vector, k,
        )
        return [
...
def _lc_to_chunk(lc_chunk: LC_Document) -> TextChunk:
    ...
    return TextChunk(
        ...
        end_char=(m.get("start_index") or 0) + len(text) if m.get("start_index") else None,
        metadata={
            "source": m.get("source"),
