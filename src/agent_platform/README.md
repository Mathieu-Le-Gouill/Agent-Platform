...
- `settings.py`, `Settings` (`pydantic-settings`, env prefix `AGENT_PLATFORM_`), `get_settings()` (`lru_cache`'d singleton). Default provider selection for each of the three wired domains is a single `"<provider>:<model>"` string (`default_llm_model`, `default_image_model`, `default_audio_model`), parsed by `model_string.py::parse_model_string`
