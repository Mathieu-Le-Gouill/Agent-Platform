...
- **Test suite**, `tests/` (1669 tests, 1666 passed + 3 skipped, structural + coverage-gate pass done): `pyproject.toml [tool.pytest.ini_options] addopts` enforces `--cov-fail-under=90` locally and in CI (config lives in `pyproject.toml [tool.coverage.*]`, no `.coveragerc`); actual coverage is 97.67%. Mocking goes through the `pytest-mock` `mocker` fixture, not raw `unittest.mock.patch`; every test is auto-marked `unit` via a `pytest_collection_modifyitems` hook in `tests/conftest.py` unless explicitly marked `integration` (currently 0 tests are, the suite is fully mocked); shared helpers live in `tests/helpers/`. See AGENTS.md §3 for the full convention

### Known issues

None currently tracked. The previous P0-P3 backlog (RAG pipeline gaps, speech translation stub, missing classification layer, missing agent tools, WhisperX diarization import failure, `chunking-pdf`/`unstructured` resolution failure, unused `classification-transformers` extra, coverage gaps in `llm_classifier`/`embed_classifier`/`similarity_scorer`/`audio`) was closed in one pass; see git history around this date for the corresponding commits. Two incidental bugs surfaced and were fixed while writing tests: `FewShotStrategy.build_system_prompt` had an inverted `multi_label` condition that crashed all single-label few-shot prompts, and `LLMClassifier`'s malformed-tool-call handler caught `agent_platform.core.errors.ValidationError` instead of `pydantic.ValidationError`, so it never actually triggered.

A separate coverage pass over `agents/`, `core/schemas/document.py`, and the vector-store/speech-to-text integrations closed three more bugs. `integrations/speech_to_text/deepgram/deepgram.py` still called the old deepgram-sdk v3 API (`DeepgramClient`, `PrerecordedOptions`, `client.listen.asyncprerecorded...`) even though `pyproject.toml` already pinned `deepgram-sdk>=7` (a full Fern-rewrite with none of those names); `transcribe()`/`stream()` would have raised `ImportError` at runtime. Rewritten against the current `AsyncDeepgramClient` / `client.listen.v1.media.transcribe_file` / `client.listen.v1.connect()` API; the now-unused `_mime_from_format` helper was removed since the new API sniffs the container format server-side. `agents/agent.py`'s `act_stream` dropped a tool's output whenever the final stream chunk wasn't an error, so non-streaming tools invoked through the streaming path always returned empty content. `core/schemas/document.py`'s `_infer_image_format`/`_infer_audio_format`/`_infer_video_format` passed a full file path to `FileFormat.from_extension()` (which expects just the extension) instead of `FileFormat.from_path()`, so format auto-detection on `load_content()` always returned `UNKNOWN`. Coverage on the touched modules is now 100%.
