...
## Current state (as of 2026-07-26, five `integrations/` gaps closed against a modern-agent-platform review: `llm`'s `stream()` now accepts `tools` and yields `ToolCallDelta` fragments across all 5 providers; `llm/openai` and `llm/anthropic` implement a new `BaseBatchLLMProvider` mixin for async batch inference; a native `llm/google`/`embeddings/google` (Gemini) provider was added; `vector_store/qdrant` gained `add_hybrid`/`search_hybrid` (dense+sparse, RRF-fused) paired with a new local `embeddings/bm25` sparse embedder; a new `moderation` domain (`openai` + a local keyword-rule fallback) was added)
